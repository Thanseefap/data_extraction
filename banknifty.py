...
for i in range(20):
    if i!=0:
        strike.append(f"BANKNIFTY{expiry}C{openstrike-100*i}")
        strike.append(f"BANKNIFTY{expiry}P{openstrike-100*i}")   
        strike.append(f"BANKNIFTY{expiry}C{openstrike+100*i}") 
        strike.append(f"BANKNIFTY{expiry}P{openstrike-100*i}")   
        #print(openstrike-100*i,openstrike+100*i)
    else:
        strike.append(f"BANKNIFTY{expiry}P{openstrike}")   
        strike.append(f"BANKNIFTY{expiry}C{openstrike}")   
strike
...
f= datetime.datetime( tod.year, tod.month-1,  tod.day+18, 9, 15, 0) 
ts = f.timestamp()
#ed=f.timestamp()
...
for i in strike:
        ...
        if df['stat'].unique()[0]=='Ok':
         ...
         #,'intoi','intv','oi','v' 
            ...
            df['Strike']=i
            df['expiry']=expiry
            df.to_sql('BANKNIFTY_OPTION_Onemin_1', conn, if_exists='append', index=False)
        else:
